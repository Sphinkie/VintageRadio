#!/usr/bin/env python3
"""
DLNA browser built with the upnpinfo library.

Features
--------
* Discover all UPnP/DLNA MediaServer devices on the current LAN.
...
# upnpinfo imports
# ----------------------------------------------------------------------
from upnpinfo import discover, Device  # discover returns a list of Device objects

# ----------------------------------------------------------------------
# Configuration constants (feel free to edit)
# ----------------------------------------------------------------------
INI_FILE = Path("preferend_dlna.ini")
DEFAULT_ROOT = "Audios"          # top‑level folder on the server
DEFAULT_SUB1 = "By Folder"       # first sub‑folder (change if your server uses another name)
DEFAULT_SUB2 = "Blues"           # second sub‑folder (change if needed)
...
# ----------------------------------------------------------------------
def discover_media_servers(timeout: int = 5) -> List[Device]:
    """
    Run an SSDP M‑SEARCH for devices of type
    urn:schemas-upnp-org:device:MediaServer:1.
    Returns a list of upnpinfo.Device objects.
    """
    # `discover` returns every device that answers the search.
    # We filter only MediaServer devices.
    all_devices = discover(st="urn:schemas-upnp-org:device:MediaServer:1",
                          timeout=timeout)
    return [dev for dev in all_devices if dev.device_type == "urn:schemas-upnp-org:device:MediaServer:1"]


# ----------------------------------------------------------------------
# Interactive server selection (fallback)
# ----------------------------------------------------------------------
def choose_server_interactively(devices: List[Device]) -> Device:
    print("\n=== Serveurs DLNA découverts ===")
    for idx, dev in enumerate(devices, start=1):
        # Friendly name is fetched lazily; we request it once here.
...
# ----------------------------------------------------------------------
def browse_to_folder(device: Device, path_parts: List[str]) -> List[dict]:
    """
    Starting at the root container (ID='0'), descend through each folder name
    supplied in ``path_parts``.  Returns the list of DIDL‑Lite items that live
    in the final container.
    """
    # The ContentDirectory service is identified by its URN.
    cd_service = device.get_service("urn:schemas-upnp-org:service:ContentDirectory:1")
    if cd_service is None:
        raise RuntimeError("Ce serveur ne propose pas de service ContentDirectory.")

    container_id = "0"                       # root container per UPnP spec
    for part in path_parts:
        # Browse the children of the current container.
        result = cd_service.browse(ObjectID=container_id,
                                   BrowseFlag="BrowseDirectChildren",
                                   Filter="*",
                                   StartingIndex=0,
                                   RequestedCount=0,
                                   SortCriteria="")
        # `result` is a dict; the DIDL‑Lite XML is under the key 'Result'.
    ...
    final_res = cd_service.browse(ObjectID=container_id,
                                 BrowseFlag="BrowseDirectChildren",
                                 Filter="*",
                                 StartingIndex=0,
                                 RequestedCount=0,
                                 SortCriteria="")
    return final_res["Result"].items
...
def filter_mp3_items(items: List[dict]) -> List[str]:
    ...
    for it in items:
        if it.title.lower().endswith(".mp3"):
            # Most items have at least one resource; we take the first.
            if it.resources:
                url = it.resources[0].uri
                mp3_list.append(f"{it.title} – {url}")
    return mp3_list
...
def main(
    ...
    chosen: Optional[Device] = None

    if preferred_url:
        # Look for a discovered device whose description URL matches the saved one.
    ...
    # --------------------------------------------------------------
    # 3️⃣ If still none, ask the user interactively
    # --------------------------------------------------------------
    if not chosen:
        chosen = choose_server_interactively(servers)

        # Offer to remember the choice for next runs
    ...
    # --------------------------------------------------------------
    # 4️⃣ Browse to the target folder
    # --------------------------------------------------------------
    try:
...
if __name__ == "__main__":
    # Feel free to change the folder names here if your server uses different ones.
    main(
        root_folder="Audios",
        subfolder1="By Folder",   # adjust as needed
        subfolder2="Blues",       # adjust as needed
    )